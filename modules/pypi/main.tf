locals {
  name         = coalesce(var.name, "pypi")
  environments = toset(["dev", "test", "prod"])

  group_names = {
    dev  = nexus_repository_pypi_group.dev.name
    test = nexus_repository_pypi_group.test.name
    prod = nexus_repository_pypi_group.prod.name
  }
}

# ── Hosted repositories: one per environment ─────────────────────────────────

resource "nexus_repository_pypi_hosted" "env" {
...
resource "nexus_repository_pypi_group" "prod" {
  ...
  group {
    member_names = [
      nexus_repository_pypi_hosted.env["prod"].name,
      nexus_repository_pypi_proxy.this.name,
    ]
  }
}

resource "nexus_repository_pypi_group" "test" {
  ...
  group {
    member_names = [
      nexus_repository_pypi_hosted.env["test"].name,
      nexus_repository_pypi_group.prod.name,
    ]
  }
}

resource "nexus_repository_pypi_group" "dev" {
  ...
  group {
    member_names = [
      nexus_repository_pypi_hosted.env["dev"].name,
      nexus_repository_pypi_group.test.name,
    ]
  }
}
...
module "roles" {
  ...
  environments = {
    for env in local.environments : env => {
      hosted_repository = nexus_repository_pypi_hosted.env[env].name
