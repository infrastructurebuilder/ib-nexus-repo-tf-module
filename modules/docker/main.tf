locals {
  name         = coalesce(var.name, "docker")
  environments = toset(["dev", "test", "prod"])

  group_names = {
    dev  = nexus_repository_docker_group.dev.name
    test = nexus_repository_docker_group.test.name
    prod = nexus_repository_docker_group.prod.name
  }
}

# ── Hosted repositories: one per environment ─────────────────────────────────

resource "nexus_repository_docker_hosted" "env" {
  ...
  docker {
    ...
    http_port        = lookup(var.docker_http_ports, each.key, null)
  }
...
resource "nexus_repository_docker_proxy" "this" {
  ...
  docker {
    force_basic_auth = var.docker_force_basic_auth
    v1_enabled       = var.docker_v1_enabled
    http_port        = lookup(var.docker_http_ports, "proxy", null)
  }

  docker_proxy {
    index_type = var.docker_proxy_index_type
    index_url  = var.docker_proxy_index_url
  }
...
#
# Note: docker group repositories are pull-only (pushing to a group requires
# a Nexus Pro writable group). Pushes go to the per-environment hosted repos.

resource "nexus_repository_docker_group" "prod" {
  ...
  docker {
    force_basic_auth = var.docker_force_basic_auth
    v1_enabled       = var.docker_v1_enabled
    http_port        = lookup(var.docker_http_ports, "prod-group", null)
  }
  ...
  group {
    member_names = [
      nexus_repository_docker_hosted.env["prod"].name,
      nexus_repository_docker_proxy.this.name,
    ]
  }
}

resource "nexus_repository_docker_group" "test" {
  ...
  docker {
    force_basic_auth = var.docker_force_basic_auth
    v1_enabled       = var.docker_v1_enabled
    http_port        = lookup(var.docker_http_ports, "test-group", null)
  }
  ...
  group {
    member_names = [
      nexus_repository_docker_hosted.env["test"].name,
      nexus_repository_docker_group.prod.name,
    ]
  }
}

resource "nexus_repository_docker_group" "dev" {
  ...
  docker {
    force_basic_auth = var.docker_force_basic_auth
    v1_enabled       = var.docker_v1_enabled
    http_port        = lookup(var.docker_http_ports, "dev-group", null)
  }
  ...
  group {
    member_names = [
      nexus_repository_docker_hosted.env["dev"].name,
      nexus_repository_docker_group.test.name,
    ]
  }
}
...
module "roles" {
  ...
  environments = {
    for env in local.environments : env => {
      hosted_repository = nexus_repository_docker_hosted.env[env].name
