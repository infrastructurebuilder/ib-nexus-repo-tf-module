output "hosted_repositories" {
  ...
  value       = { for env, repo in nexus_repository_maven_hosted.env : env => repo.name }
}
...
output "repository_info" {
  ...
  value = {
    ...
    hosted_repositories = { for env, repo in nexus_repository_maven_hosted.env : env => repo.name }
