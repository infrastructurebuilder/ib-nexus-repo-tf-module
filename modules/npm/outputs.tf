output "hosted_repositories" {
  ...
  value       = { for env, repo in nexus_repository_npm_hosted.env : env => repo.name }
}

output "proxy_repository" {
  description = "Name of the proxy repository."
  value       = nexus_repository_npm_proxy.this.name
}
...
output "repository_info" {
  ...
  value = {
    ...
    hosted_repositories = { for env, repo in nexus_repository_npm_hosted.env : env => repo.name }
